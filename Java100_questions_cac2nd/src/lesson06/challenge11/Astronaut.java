...
class Gammalian {
	...
	public void setItem(String item) {
		boolean isHit = false;//文字列に数字が含まれているか検査している処理
		for (int i = 0; i < 10; i++) {
			if (item.indexOf(Integer.toString(i)) != -1) {
				//文字列の中に数字があるかチェックしている条件
				isHit = true;
				break;
			}
		}
...
public class Astronaut {
	...
	public static void main(String[] args) throws IOException {
		...
		//オブジェクト生成
		gammalian.setItem(present);//ここでfalseかtrueかをセットのところを一度通して判断している

		boolean isGood = false;
		isGood = gammalian.isGood();//その結果をisGoodに入れている
